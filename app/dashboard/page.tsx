...
export default function DashboardPage() {
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <RightPanel
        isReady={status?.ready}
        rowsTotal={summary?.error_rates?.total_rows}
        maintenanceSites={summary?.maintenance_sites?.maintenance_site_count}
      />

      {/* Main Content */}
      <div className="ml-64 mr-80 pt-24 pb-12 px-8">
          ...
          {status?.ready && summary && !isLoading ? (
            <>
              ...
              <section data-animate className="grid gap-6 lg:grid-cols-2">
                {/* Bill Range */}
                <div className="rounded-2xl border border-border/40 bg-card/50 backdrop-blur p-6">
                  ...
                  <div className="space-y-4">
                    {summary.site_types?.PEA && (
                      <div className="rounded-lg bg-success/10 border border-success/20 p-4">
                        <p className="text-sm font-medium mb-3">PEA</p>
                    ...
                    {summary.site_types?.MEA && (
                      <div className="rounded-lg bg-warning/10 border border-warning/20 p-4">
                        <p className="text-sm font-medium mb-3">MEA</p>
                        <div className="space-y-2">
              ...
              <section data-animate>
                ...
                <div className="mb-4 rounded-2xl border border-border/40 bg-card/50 backdrop-blur p-4">
                  <SiteSearch />
                </div>
                {selectedSiteId && trend ? (
                  <TrendChart trend={trend} />
                ) : (
                  <div className="rounded-2xl border border-border/40 bg-card/50 backdrop-blur p-12 text-center">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border/40 bg-surface/30">
                        <th className="px-6 py-3 text-left font-semibold text-xs uppercase">
                          Site ID
                        </th>
                        <th className="px-6 py-3 text-left font-semibold text-xs uppercase">
                          Provider
                        ...
                        <th className="px-6 py-3 text-right font-semibold text-xs uppercase">
                          Amount
                        </th>
                        <th className="px-6 py-3 text-left font-semibold text-xs uppercase">
                          Last Maintenance
